...
3. **실행 확인** : 프로그램 설치와 환경변수까지 제대로 설정되었는지 확인하기 위해 [Running the simulator](https://isaac-orbit.github.io/orbit/source/setup/installation.html#running-the-simulator)에 안내되어 있는 몇가지 점검 command를 실행시켜 보겠습니다.
    ...
    - 시뮬레이터 내부 파이썬 실행 파일(`python.sh`) 확인
      ​    
        ...
        ![cmdq](../../images/2023-04-04-insatll-orbit/cmdq.png)
    ...
    - 시뮬레이터 standalone 예제(`add_cubes.py`)로 실행 확인
        ...
        ![cube](../../images/2023-04-04-insatll-orbit/cube.png)
...
![orbit](../../images/2023-04-04-insatll-orbit/orbit.png)

# Orbit에서 가상환경 설정

Orbit에서는 `orbit -p`라는 명령어를 통해 Isaac Sim의 파이썬 실행파일을 자동으로 가져와서 사용합니다. (혹은 위에서 명령어 별칭을 등록안했다면 Orbit repo 위치에서 `./orbit.sh -p`라고 입력해도 됩니다.) 하지만 가상환경을 사용하고 싶다면 Conda를 이용하면 됩니다.

4. `sudo apt install cmake build-essential`

5. `conda create -n orbit python=3.8`

6. `orbit --install`

7. `orbit --extra`

8. `orbit -p source/standalone/demo/play_empty.py`

[1] https://forums.developer.nvidia.com/t/issac-sim-vs-orbit/244656