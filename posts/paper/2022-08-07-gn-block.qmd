...
이번 post는 [Graph Networks as Learnable Physics Engines for Inference and Control](https://arxiv.org/abs/1806.01242) 라는 논문을 읽고 리뷰한 내용입니다.
...
![](https://i.imgur.com/yN3GlTK.png?1)
...
![](https://i.imgur.com/TjghKvm.png?1)
...
![](https://i.imgur.com/2tN81Kd.png?1)
...
![](https://i.imgur.com/7ZKlooE.png?1)
...
![](https://i.imgur.com/Snfkuwb.png?1)
...
![](https://i.imgur.com/4ZMFGU1.png?1)
...
1. MPC(Model Predictive Control)
    ...
    ![](https://i.imgur.com/4s443v3.png?1)
...
- generalization and system identification 실험을 위해서
    - created a dataset of versions of several of our systems—Pendulum, Cartpole, Swimmer, Cheetah and JACO— with procedurally varied parameters and structure.
    - 변화시킨 속성들로는 link lengths, body masses, and motor gears. + varied the number of links in the Swimmer’s structure, from 3-15 (we refer to a swimmer with N links as SwimmerN )

<span style="color: blue">MPC planning</span>

- **N-step trajectory**(N: planning horizon)와 그 trajectory를 실행했을 때 받을 **total reward**를 GN forward 모델로 예측했다.
- 이때의 action sequences(=trajectory)들을 total reward의 backpropagating gradient를 가지고 최적화하게 된다.

Model-based reinforcement learning

- GN-based model을 강화학습에 적용해보다
- SVG를 이용했다.
- GN forward model이 미분가능하기 때문에 GN 모델로 생성된 next state를 가지고 expected return 값의 그라디언트를 구할 수 있다.
- 1 step을 예측하는 SVG(1)과
- model-free RL 베이스라인과 비교하기 위해 SVG(0)과 deterministic policy 알고리즘인 DDPG(Deep Deterministic Policy Gradients)알고리즘을 사용했다.

<span style="color: blue">Baseline comparisons</span>

1. constant prediction baseline: input state를 그대로 output state로 사용
2. MLP baseline: GN model에 쓰인 데이터를 그대로 MLP에 flattened & concatenated 해서 학습
3. MPC baseline: 물리 모델을 가지고 Differential Dynamic Programming algorithm을 사용해서 ground truth 값을 가짐
...
- random control로 만들어진 데이터들을 가지고 학습된 GN-based model
    - **[Visually]** Swimmer6에서 그림에서 처럼 ground truth와 예측 결과가 구분이 안 갈 정도로 흡사하다.([영상](https://drive.google.com/file/d/1ez6sqCTZmHdSnf86lK6NEsnzyi-O25-B/view)에서도 거의 구분이 안 갈 정도로 잘 예측하고 있음을 알 수 있다.)
