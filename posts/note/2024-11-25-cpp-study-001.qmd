...
{
// 복잡한 시스템을 ros2_control에 통합할 때 구현해야 하는 가상 클래스
/**
 * 이 클래스는 다관절 로봇이나 센서를 포함하는 시스템 등을 ros2_control에 통합하기 위해 사용됩니다.
 * 
 * 하드웨어의 상태는 다음과 같은 단계로 나눌 수 있습니다:
 *
 * - UNCONFIGURED (on_init, on_cleanup): 하드웨어는 초기화되었으나 통신은 시작되지 않은 상태
 * - INACTIVE (on_configure, on_deactivate): 하드웨어가 설정되고 통신이 시작되었으나 움직임 제어는 비활성화된 상태
 * - FINALIZED (on_shutdown): 하드웨어 인터페이스가 언로드 또는 파괴를 준비하는 상태
 * - ACTIVE (on_activate): 하드웨어가 활성화되고 실제로 움직일 수 있는 상태
 */
...
public:
  ...
  // 복사 생성자를 비활성화 (고유 소유권을 가지므로 복사 불가능)
  ...
  // 하드웨어 초기화를 위한 가상 메소드 (하위 클래스에서 구현)
  ...
  // URDF 파일에 정의되지 않은 사용자 정의 상태 인터페이스를 내보내기 위한 메소드
...
소스코드를 이해하기 위한 10가지 사항들을 확인해보며 c++ 코드를 공부합니다.

### 1. **스마트 포인터 사용 (`std::shared_ptr`, `std::unique_ptr`)**
스마트 포인터인 `std::shared_ptr`는 하드웨어 인터페이스와 관련된 상태와 명령 인터페이스를 관리하는 데 사용됩니다.
```cpp
std::vector<StateInterface::ConstSharedPtr> on_export_state_interfaces()
{
  auto state_interface = std::make_shared<StateInterface>(description);
...
### 2. **복사 생성자 삭제 (`= delete`)**
복사 생성자를 명시적으로 삭제하여 객체가 복사되지 않도록 합니다.
```cpp
SystemInterface(const SystemInterface & other) = delete;
```

### 3. **이동 생성자 (`= default`)**
이동 생성자를 기본값으로 설정하여 객체 이동을 허용하지만 복사는 방지합니다.
```cpp
SystemInterface(SystemInterface && other) = default;
```

### 4. **`virtual` 키워드와 가상 함수**
`virtual` 키워드를 사용하여 하위 클래스에서 재정의 가능한 메서드를 정의합니다.
```cpp
virtual CallbackReturn on_init(const HardwareInfo & hardware_info)
{
  // 초기화 로직
...
```
또한 가상 소멸자를 사용하여 상속된 클래스에서도 올바르게 소멸자를 호출할 수 있도록 합니다.
```cpp
virtual ~SystemInterface() = default;
```

### 5. **`[[deprecated]]` 속성**
특정 메서드를 더 이상 권장되지 않음을 나타내기 위해 `[[deprecated]]` 속성을 사용합니다.
```cpp
[[deprecated(
  "Replaced by vector<StateInterface::ConstSharedPtr> on_export_state_interfaces() method. "
...
### 6. **C++11의 `override` 키워드**
하위 클래스에서 기본 클래스의 가상 함수를 재정의할 때 `override` 키워드를 사용하여 재정의를 명확히 합니다. (이 코드에서는 `override`는 없지만 이를 사용하는 것이 좋은 방법입니다.)
...
### 7. **`const`와 `&`의 사용**
객체의 멤버를 변경하지 않도록 보장하기 위해 `const`를 사용하고, 참조로 전달하여 복사 비용을 줄입니다.
```cpp
virtual std::string get_name() const { return info_.name; }
...
### 8. **언어 확장 속성 (`[[nodiscard]]`, `[[deprecated]]` 등)**
이 코드에서는 `[[deprecated]]`를 사용하여 메서드가 더 이상 권장되지 않음을 나타내고 있습니다.
```cpp
[[deprecated(
  "Replaced by vector<StateInterface::ConstSharedPtr> on_export_state_interfaces() method. "
  "Exporting is handled by the Framework.")]]
...
### 9. **다중 반환 방식 (CallbackReturn)**
`CallbackReturn` 타입을 사용하여 함수가 여러 가지 결과 상태를 반환하도록 합니다.
```cpp
CallbackReturn init(
  const HardwareInfo & hardware_info, rclcpp::Logger logger,
  rclcpp::node_interfaces::NodeClockInterface::SharedPtr clock_interface)
...
### 10. **`std::unordered_map`의 사용**
여러 인터페이스를 효율적으로 관리하기 위해 `std::unordered_map`을 사용합니다.
```cpp
std::unordered_map<std::string, InterfaceDescription> joint_state_interfaces_;
std::unordered_map<std::string, CommandInterface::SharedPtr> system_commands_;
```
...
<!--
### 11. **코딩 스타일 팁**
접근 제한자(`public`, `protected`, `private`)를 명확히 구분하여 각 레벨의 접근을 관리합니다.
```cpp
public:
  SystemInterface();
...
private:
  ...
  rclcpp::Logger system_logger_;
```
또한 함수의 역할과 반환 값을 설명하는 주석을 충분히 사용하고 있습니다.
```cpp
/// 하드웨어 인터페이스의 이름을 반환하는 메소드
/**
 * \return 하드웨어의 이름을 반환합니다.
 */
virtual std::string get_name() const { return info_.name; }
```

이와 같은 코딩 팁과 C++의 특별한 문법을 활용하면 코드의 안전성, 유지보수성, 그리고 가독성을 크게 향상시킬 수 있습니다.
