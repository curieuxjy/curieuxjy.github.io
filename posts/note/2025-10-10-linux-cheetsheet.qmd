...
# img rezise (blog posting)

- 50MB 넘는 파일 파악

```
find ./images -type f -size +50M -exec du -h {} + | sort -h
```
...
```bash
# ImageMagick이 설치되어 있을 때 (가로x세로 출력)
...
```bash
# 예: 1280x720 -> 640x360
gifsicle --resize 640x360 input.gif -o out_resized.gif
```
...
```bash
# 가로만 640으로 맞추고 비율 유지
gifsicle --resize-width 640 input.gif -o out_resized_w640.gif

# 세로만 360으로 맞추고 비율 유지
gifsicle --resize-height 360 input.gif -o out_resized_h360.gif
```

4. 해상도 축소 + 색상 수 줄이기 + 최고 수준 최적화 (권장)

```bash
gifsicle --resize 640x360 --colors 64 -O3 input.gif -o out_optimized.gif
```

* `--colors 64`는 팔레트(색상) 수를 64로 제한합니다. 숫자를 더 낮추면 용량은 더 줄지만 색상 열화가 생깁니다.
* `-O3`는 최적화 레벨(최대)입니다.
...
- option
  - `-v` : 일치되는 내용이 **없는** **라인**을 표시
  - `-c` : 일치되는 내용이 있는 **행의 개수**를 표시
  - `-l` : 일치되는 내용이 있는 **파일 이름**만 표시
...
1. appimage 파일과 icon image를 `/opt`로 이동
   - e.g. `/opt/[app_name].AppImage` & `/opt/[app_name].png`
2. 바로가기 설정 파일 `[app_name.desktop]`을 `/usr/share/applications`에 만들어줌
  ```{.sh code-line-numbers=false}
  [Desktop Entry]
  Name=[app_name]
...
- Solution: `export LD_LIBRARY_PATH=/home/avery/anaconda3/envs/[가상환경이름]/lib`
  - path는 `conda env list`로 구체적인 해당 경로 확인 필요. mamba나 miniforge3을 사용할 경우 경로가 다를 수 있음.
...
- 원래 마운트 지점을 공유로 표시
  ...
  - 선택한 마운트 지점과 그 아래의 모든 마운트 지점에 대한 마운트 유형을 `변경`하려면 `--make-shared` 대신 `--make-r shared` 옵션 사용
- 중복 생성
